...
def my_start():
    ...
    if userSearch == "":
        print("You can't leave the search emtpy!\nTry Again...")
        my_start()
    ...
    if linkPattern in userSearch:
        ...
        url = "https://www.youtube.com/results?search_query=" + userSearch

        html = urllib.request.urlopen(url.replace(chr(228), "ae").replace(chr(196), "Ae").replace(chr(246), "oe").replace(chr(214), "Oe").replace(chr(252), "ue").replace(chr(220), "Ue"))
...
linkPattern = "www.youtube.com"
helpPattern = "help"
userSearch = (input("What are you looking for? \n>> ")).replace(" ", "+").replace("ä", "ae").replace("ö", "oe").replace("ü", "ue").replace("Ä", "Ae").replace("Ö", "Oe").replace("Ü", "Ue")
